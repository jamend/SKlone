/****** Object:  Stored Procedure dbo.MissileFusion    Script Date: 5/1/2004 6:12:39 PM ******/
CREATE PROCEDURE dbo.MissileFusion
	(
		@Attacker int,
		@Defender int,
		@SKDate nvarchar(24),
...
AS
	...
	ELSE
			BEGIN
				...
				END
					ELSE
					IF (SELECT Fusion FROM Missiles WHERE kdID = @Attacker) = 0
						BEGIN
						SET @Result = 'You are out of Fusion Missiles.'
						END
					ELSE
					BEGIN
						DECLARE @AttackerName nvarchar(48)
						DECLARE @DefenderName nvarchar(48)
						DECLARE @Fusion tinyint
						SET @Fusion = (SELECT Fusion FROM Missiles WHERE kdID = @Attacker)
						...
						BEGIN
							...
							SET @LandGain = dbo.FusionLoss(@Attacker, @Defender)
							SET @PopulationGain = (SELECT Population FROM Kingdoms WHERE kdID = @Defender) * 0.06
							EXEC dbo.BuildingLossC @Attacker, @Defender
							...
							UPDATE Missiles SET Fusion = Fusion - 1 WHERE kdID = @Attacker
							...
							SET @AttackerSectorNews = 'On ' + @SKDate + ', our brave kingdom ' + @AttackerName + ' launched a <b>fusion missile</b> at  ' + @DefenderName + ' and destoried <b>' + dbo.FormatNumber(@LandGain) + '</b> land.'
							SET @DefenderSectorNews = 'On ' + @SKDate + ', our kingdom ' + @DefenderName + ' was hit with a <b>fusion missile</b> from ' + @AttackerName + ' and lost <b>' + dbo.FormatNumber(@LandGain) + '</b> land.'
							SET @PersonalNews = 'Our planet has been hit by a <b>fusion missile</b> from ' + @AttackerName + ' . We have lost <b>' + dbo.FormatNumber(@LandGain) + '</b> land,  and <b>' + dbo.FormatNumber(@PopulationGain) + '</b> population.<br>'
							SET @Result = 'The missile launch is complete. Our officers have reported that our <b>fusion missile</b> has destoried <b>' + dbo.FormatNumber(@LandGain) + '</b> land,  and killed <b>' + dbo.FormatNumber(@PopulationGain) + '</b> population.<br>'
						END
