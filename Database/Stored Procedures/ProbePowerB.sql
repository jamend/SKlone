/****** Object:  Stored Procedure dbo.ProbePowerB    Script Date: 5/1/2004 6:12:40 PM ******/
CREATE PROCEDURE dbo.ProbePowerB
	(
		@Attacker int,
		@Defender int,
		@Probes bigint,
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							UPDATE Kingdoms SET [Power] = [Power] - (@Probes * 2) WHERE kdID = @Attacker
							SET @PowerLoss = dbo.PowerLoss(@Attacker, @Defender, @Probes)
							UPDATE Buildings SET Built  = Built - @PowerLoss  WHERE (BuildingType = 3) AND kdID = @Defender
							UPDATE Kingdoms SET Probes = Probes - @ProbesLostA WHERE kdID = @Attacker
							UPDATE Scores SET PPBurned = PPBurned + @PowerLoss WHERE kdID = @Attacker
							SET @PersonalNews = 'Our police report that probes have destroyed <b>' + dbo.FormatNumber(@PowerLoss) + '</b> <b>powerplants</b>.  '
							IF @ResearchPercent >= 20 SET @PersonalNews = @PersonalNews + '  Our intelligance reports that they came from  <b>' + @AttackerName + '</b>.'
							SET @Result = 'Your probes have  <b>successfully</b> destroyed some of the enemies powerplants. The computer reports that they burned <b>' + dbo.FormatNumber(@PowerLoss) + '</b> <b>powerplants</b>. Unfortunantly we lost <b>' + dbo.FormatNumber(@ProbesLostA) + '</b> <b>probes</b> in this mission. '
							INSERT INTO PersonalNews(kdID, NewsText) VALUES (@Defender, @PersonalNews)
					END
				END
			END
