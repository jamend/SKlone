/****** Object:  Stored Procedure dbo.ProbePopB    Script Date: 5/1/2004 6:12:40 PM ******/
CREATE PROCEDURE dbo.ProbePopB
	(
		@Attacker int,
		@Defender int,
		@Probes bigint,
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							SET @PopulationGain = dbo.PopulationGain(@Attacker, @Defender,  @Probes)
							UPDATE Kingdoms SET [Power] = [Power] - (@Probes * 2) WHERE kdID = @Attacker
							UPDATE Kingdoms SET Population = Population + @PopulationGain WHERE kdID = @Attacker
							...
							UPDATE Scores SET PopStolen = PopStolen + @PopulationGain WHERE kdID = @Attacker
							SET @PersonalNews = 'Our police report that probes have kidnaped <b>' + dbo.FormatNumber(@PopulationGain) + '</b> <b>civilians</b>. '
							IF @ResearchPercent >= 20 SET @PersonalNews = @PersonalNews + '  Our intelligance reports that they came from  <b>' + @AttackerName + '</b>.'
							SET @Result = 'Your probes have  <b>successfully</b> kidnaped some civilians. The computer reports that they kidnaped <b>' + dbo.FormatNumber(@PopulationGain) + '</b> <b>civilians</b>. Unfortunantly we lost <b>' + dbo.FormatNumber(@ProbesLostA) + '</b> <b>probes</b> in this mission. '
							INSERT INTO PersonalNews(kdID, NewsText) VALUES (@Defender, @PersonalNews)
					END
				END
			END
