/****** Object:  Stored Procedure dbo.Attack2    Script Date: 5/1/2004 6:12:40 PM ******/
CREATE PROCEDURE dbo.Attack2
	(
		@Attacker int,
		@Defender int,
		@WLs tinyint,
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						IF @AttackPower > 0
						BEGIN
							SET @DefenderLoss = 0.08894 / (CAST(@DefencePower AS real) / @AttackPower)
						END
						ELSE
						BEGIN
							SET @DefenderLoss = 0
						...
						UPDATE Kingdoms SET Attacks = Attacks + 1 WHERE kdID = @Defender
						IF (SELECT Attacks FROM Kingdoms WHERE kdID = @Defender) > 2
						BEGIN
						...
						SET @Result = @Result + 'and <b>' + dbo.FormatNumber(@TanksLostA) + '</b> tanks. We have killed about <b>' + dbo.FormatNumber(ROUND(@SoldiersLostB + @LTroopersLostB + @LDragoonsLostB + @TanksLostB + @TFsLostB, -1)) + '</b> enemy military units.<br>Our attacking troops will take <b>' + dbo.FormatNumber(@ReturnTime) + '</b> hours to return home.'
						print @Result
					END
				END
			END
