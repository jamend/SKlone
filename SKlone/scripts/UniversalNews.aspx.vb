...
#End Region
    ...
    Private Sub ViewNews(ByVal NewsDate As Int32)
        ...
        SqlData1.Read()
        Dim PublicID As Int32 = SqlData1.GetInt32(0)
        ...
        Select Case NewsDate
            Case 0
                SelectCommand = "SELECT '<img src=/images/' + CAST(NewsType AS nvarchar(1)) + '.gif>' AS img, NewsDate, NewsText FROM UniversalNews WHERE DAY(NewsDate) = DAY(GETDATE()) AND PublicID = " & PublicID & " ORDER BY NewsDate"
            Case 1
                SelectCommand = "SELECT '<img src=/images/' + CAST(NewsType AS nvarchar(1)) + '.gif>' AS img, NewsDate, NewsText FROM UniversalNews WHERE DAY(NewsDate) = DAY(DATEADD(day, -1, GETDATE())) AND PublicID = " & PublicID & " ORDER BY NewsDate"
            Case 2
                SelectCommand = "SELECT '<img src=/images/' + CAST(NewsType AS nvarchar(1)) + '.gif>' AS img, NewsDate, NewsText FROM UniversalNews WHERE DAY(NewsDate) = DAY(DATEADD(day, -2, GETDATE())) AND PublicID = " & PublicID & " ORDER BY NewsDate"
            Case 3
                SelectCommand = "SELECT '<img src=/images/' + CAST(NewsType AS nvarchar(1)) + '.gif>' AS img, NewsDate, NewsText FROM UniversalNews WHERE DAY(NewsDate) = DAY(DATEADD(day, -3, GETDATE())) AND PublicID = " & PublicID & " ORDER BY NewsDate"
        End Select
        ...
        If SqlData1.HasRows = False Then
            SqlAdapter1.SelectCommand = New SqlClient.SqlCommand("SELECT ' ' AS img, ' ' AS NewsDate, 'No Universal News Today' AS NewsText", Session.Item("sqlConnection"))
        Else
