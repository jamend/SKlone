...
#End Region
        ...
        Private Sub ViewNews(ByVal NewsDate As Int32)
            ...
            If SqlData1.IsDBNull(0) = True Then
                ...
                lblNoAlliance.Visible = True
                DataGrid1.Visible = False
            Else
                Dim AllianceID As Int32 = SqlData1.GetInt32(0)
                Session.Item("sqlConnection").close()
                lblNoAlliance.Visible = False
                ...
                Select Case NewsDate
                    Case 0
                        SelectCommand = "SELECT '<img src=/images/' + CAST(NewsType AS nvarchar(1)) + '.gif>' AS img, NewsDate, NewsText FROM AllianceNews WHERE DAY(NewsDate) = DAY(GETDATE()) AND AllianceID = " & AllianceID & " ORDER BY NewsDate"
                    Case 1
                        SelectCommand = "SELECT '<img src=/images/' + CAST(NewsType AS nvarchar(1)) + '.gif>' AS img, NewsDate, NewsText FROM AllianceNews WHERE DAY(NewsDate) = DAY(DATEADD(day, -1, GETDATE())) AND AllianceID = " & AllianceID & " ORDER BY NewsDate"
                    Case 2
                        SelectCommand = "SELECT '<img src=/images/' + CAST(NewsType AS nvarchar(1)) + '.gif>' AS img, NewsDate, NewsText FROM AllianceNews WHERE DAY(NewsDate) = DAY(DATEADD(day, -2, GETDATE())) AND AllianceID = " & AllianceID & " ORDER BY NewsDate"
                    Case 3
                        SelectCommand = "SELECT '<img src=/images/' + CAST(NewsType AS nvarchar(1)) + '.gif>' AS img, NewsDate, NewsText FROM AllianceNews WHERE DAY(NewsDate) = DAY(DATEADD(day, -3, GETDATE())) AND AllianceID = " & AllianceID & " ORDER BY NewsDate"
                End Select
                SqlCommand1.CommandText = SelectCommand
                Session.Item("sqlConnection").Open()
                SqlData1 = SqlCommand1.ExecuteReader
                SqlData1.Read()
                If SqlData1.HasRows = False Then
                    SqlAdapter1.SelectCommand = New SqlClient.SqlCommand("SELECT ' ' AS img, ' ' AS NewsDate, 'No Alliance News Today' AS NewsText", Session.Item("sqlConnection"))
                Else
