Namespace scripts
    Partial Class AggressiveProbings
...
#End Region
        ...
        Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            Dim SqlCommand1 As New SqlClient.SqlCommand("SELECT TOP 100 dbo.FullKingdomNameC(kdID), PopStolen, MoneyRobbed, HomesBurned, PPBurned FROM Scores WHERE (PopStolen + MoneyRobbed + HomesBurned + PPBurned) > 0 ORDER BY (PopStolen + MoneyRobbed + HomesBurned + PPBurned) DESC", Session.Item("sqlConnection"))
            ...
            While SqlData1.Read()
                ...
                tblScores.Rows(x).Cells(2).Text = SqlData1.GetInt32(1).ToString(NumberFormatString)
                tblScores.Rows(x).Cells(2).HorizontalAlign = HorizontalAlign.Center
                tblScores.Rows(x).Cells(3).Text = SqlData1.GetInt32(2).ToString(NumberFormatString)
                tblScores.Rows(x).Cells(3).HorizontalAlign = HorizontalAlign.Center
                tblScores.Rows(x).Cells(4).Text = SqlData1.GetInt32(3).ToString(NumberFormatString)
                tblScores.Rows(x).Cells(4).HorizontalAlign = HorizontalAlign.Center
                tblScores.Rows(x).Cells(5).Text = SqlData1.GetInt32(4).ToString(NumberFormatString)
                tblScores.Rows(x).Cells(5).HorizontalAlign = HorizontalAlign.Center
                x += 1
            End While
